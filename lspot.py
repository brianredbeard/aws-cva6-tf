...
def get_spot_prices(instance_type, regions, verbose):
    ...
    for region in regions:
        ...
        if prices['SpotPriceHistory']:
            ...
            min_spot_price = min(az_prices.values(), key=lambda x: x if x != 'N/A' else float('inf'))
            min_az = next((az for az, price in az_prices.items() if price == min_spot_price), 'N/A')
...
def output_table(spot_prices):
    ...
    df = df.sort_values(by='MinSpotPrice', ascending=True)  # Sort by MinSpotPrice in ascending order
...
def output_single_lowest(spot_prices, verbose=False):
    ...
    logger(f"Region: {min_price_entry['Region']}, AZ: {min_price_entry['MinSpotPriceAZ']}, Price: {min_price_entry['MinSpotPrice']}", verbose)
    print(f"{min_price_entry['MinSpotPriceAZ']}")

def main():
    parser = argparse.ArgumentParser(description='Check AWS EC2 spot prices for a given instance type.')
    parser.add_argument('instance_type', type=str, help='The instance type to check spot prices for.')
    parser.add_argument('--json', action='store_true', help='Output in JSON format.')
    parser.add_argument('--table', action='store_true', help='Output in table format (default).')
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose output.')
    parser.add_argument('-1', '--one', action='store_true', help='Output the single lowest cost availability zone.')
    parser.add_argument('-r', '--region', action='append', help='Specify regions to check (can be used multiple times).')
